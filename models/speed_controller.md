...
cannot be committed and regenerated here the way `src/speed_controller/`
normally would be. This document instead describes, in words, the block
diagram that `src/speed_controller/speed_controller.c` corresponds to, and
the generated C sources were hand-authored to faithfully match the file
and comment structure Simulink Embedded Coder actually emits (banner
comments, `ExtU_`/`ExtY_`/`DW_`/`P_` structs, `<model>_initialize` /
`_step` / `_terminate` entry points) -- they are **not** real Embedded
Coder output. See `README.md` for why this module is traced differently
from the other six (hand-written) modules.
...
- REQ-7 / ARCH-7 (`docs/architecture.sdoc`, `docs_needs/architecture.rst`)
  reference `src/speed_controller/speed_controller.c` and `.h` directly
  via a **doc-side `TYPE: File` relation**, not an in-file
  `@relation(...)` marker -- the generated files must not be hand-edited,
  so nothing should be added to them by hand, including traceability
  comments. `tests/speed_controller_test.cpp`, which *is* hand-written,
  still carries the usual `@relation(ARCH-7, ..., role=Test)` marker.
